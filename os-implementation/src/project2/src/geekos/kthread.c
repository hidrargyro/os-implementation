    ...
    struct Kernel_Thread* kthread, struct User_Context* userContext)
{
	...
	/*
	 * Step 2: Setup thread stack
	 */
	Push(kthread, (ulong_t) userContext->dsSelector);		//data selector
	Push(kthread, userContext->stackPointerAddr);			//stack pointer (end of data memory)
	Push(kthread, EFLAGS_IF);								//EFLAGs, set IF bit (thread can be interrupted)
	...
	Push(kthread, 0);										//edi
	Push(kthread, 0);										//ebp
	Push(kthread, userContext->dsSelector);					//DS register
	Push(kthread, userContext->dsSelector);					//ES register
	Push(kthread, userContext->dsSelector);					//FS register
	Push(kthread, userContext->dsSelector);					//GS register
}
